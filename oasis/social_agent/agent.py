...
class SocialAgent(ChatAgent):
    ...
    async def perform_action_by_jev(self, jev_response=None):
        """Perform action using Jev instead of a generative LLM.

        Args:
            jev_response: Pre-computed SystemOneResponse from a batched
                          Jev call. If None, falls back to making an
                          individual call.
        """
        ...
        # If no pre-computed response, make an individual call (fallback)
        if jev_response is None:
            import os
            api_key = os.getenv("TYPESAFE_API_KEY")
            if not api_key:
                agent_log.info(
                    f"Agent {self.social_agent_id} falling back to LLM "
                    f"(no TYPESAFE_API_KEY)"
                )
                return await self.perform_action_by_llm()

            state, questions = self._build_jev_request(env_prompt)
            try:
                async with AsyncTypeSafeClient() as client:
                    jev_response = await client.system_one(
                        state=state, questions=questions
                    )
            except Exception as e:
                agent_log.error(f"Agent {self.social_agent_id} Jev error: {e}")
                return await self.perform_action_by_llm()

        try:
            chosen_action = jev_response.choices["action"].choice
            agent_log.info(
                f"Agent {self.social_agent_id} Jev picked action: "
                f"{chosen_action}"
            )

            generative_actions = [
                "create_post", "create_comment", "quote_post",
                "report_post", "send_to_group", "interview"
            ]

            if chosen_action in generative_actions:
                # Provide a more constrained prompt to the generative LLM
                user_msg = BaseMessage.make_user_message(
                    role_name="User",
                    content=(
                        f"You have decided to perform the action "
                        f"'{chosen_action}'. Please execute this action "
                        f"appropriately based on your environment: "
                        f"{env_prompt}. "
                        f"Use the {chosen_action} tool and provide the "
                        f"necessary text."
                    ))
                agent_log.info(
                    f"Agent {self.social_agent_id} delegating "
                    f"{chosen_action} to LLM."
                )
                llm_response = await self.astep(user_msg)

                for tool_call in llm_response.info.get('tool_calls', []):
                    agent_log.info(
                        f"Agent {self.social_agent_id} LLM performed "
                        f"action: {tool_call.tool_name} with args: "
                        f"{tool_call.args}"
                    )
                return llm_response
            else:
                # Deterministic execution for System 1 reflexes
                # (Zero LLM Tokens)
                if chosen_action in ["do_nothing", "refresh", "trend"]:
                    return await self.perform_action_by_data(chosen_action)
                elif chosen_action in [
                    "like_post", "repost", "unlike_post", "dislike_post",
                    "undo_dislike_post"
                ]:
                    try:
                        score_val = jev_response.scores.get("target_post_id")
                        post_id = (
                            int(score_val.score)
                            if score_val and score_val.score is not None else 1
                        )
                        if post_id <= 0:
                            post_id = 1
                        return await self.perform_action_by_data(
                            chosen_action, post_id=post_id
                        )
                    except (ValueError, TypeError, AttributeError) as e:
                        agent_log.warning(
                            f"Agent {self.social_agent_id} could not parse "
                            f"post_id for {chosen_action}, defaulting: {e}"
                        )
                        return await self.perform_action_by_data(
                            chosen_action, post_id=1
                        )
                elif chosen_action in [
                    ...
                    try:
                        ...
                        if comment_id <= 0:
                            comment_id = 1
                        return await self.perform_action_by_data(
                            chosen_action, comment_id=comment_id
                        )
                    except (ValueError, TypeError, AttributeError) as e:
                        agent_log.warning(
                            f"Agent {self.social_agent_id} could not parse "
                            f"comment_id for {chosen_action}, defaulting: {e}"
                        )
                        return await self.perform_action_by_data(
                            chosen_action, comment_id=1
                        )
                elif chosen_action in ["follow", "unfollow", "mute", "unmute"]:
                    try:
                        score_val = jev_response.scores.get("target_post_id")
                        user_id = (
                            int(score_val.score)
                            if score_val and score_val.score is not None else 0
                        )
                        return await self.perform_action_by_data(
                            chosen_action,
                            followee_id=(
                                user_id if "follow" in chosen_action else None
                            ),
                            mutee_id=(
                                user_id if "mute" in chosen_action else None
                            )
                        )
                    except (ValueError, TypeError, AttributeError) as e:
                        agent_log.warning(
                            f"Agent {self.social_agent_id} could not parse "
                            f"user_id for {chosen_action}: {e}"
                        )
                        return await self.perform_action_by_data("do_nothing")

        except Exception as e:
            agent_log.error(f"Agent {self.social_agent_id} Jev error: {e}")
            return await self.perform_action_by_llm()

    def _build_jev_request(self, env_prompt: str):
        """Build the state and questions for a Jev system_one call.

        Returns:
            Tuple of (state: str, questions: dict)
        """
        from typesafe_sdk import Choice, Noul, Score

        state = (
            f"You are a social media user. Your persona is:\n"
        ...
        questions = {
            "action": Choice(
                criteria={action: None for action in available_actions},
                instructions="What action do you want to take?"
            ),
            "will_remember": Noul(
                instructions="Does this violate expectations enough to store "
                             "in long-term memory?"
            ),
            "stance_change": Score(
                criteria=["-100", "100"],
                instructions="How much does this change your stance? "
                             "(-100 to 100)"
            ),
            "target_post_id": Score(
                criteria=["0", "999999999"],
                instructions="If you are interacting with a specific post, "
                             "what is its ID? Otherwise 0."
            )
        }
