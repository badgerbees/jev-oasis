...
class OasisEnv:
    ...
    async def _perform_llm_action(self, agent):
        ...
        async with self.llm_semaphore:
            return await agent.perform_action_by_jev()

    async def _perform_batched_jev(self, agents):
        r"""Batch all Jev system_one calls through a single persistent
        AsyncTypeSafeClient, then dispatch results to each agent.

        This avoids creating a new HTTP connection per agent and fires
        all requests concurrently over a shared connection pool.
        """
        import os
        ...
        if not api_key:
            env_log.warning("No TYPESAFE_API_KEY set, falling back all "
                            "agents to LLM.")
            tasks = [self._perform_llm_action_pure_llm(agent)
                     for agent in agents]
            return await asyncio.gather(*tasks)

        # Phase 1: Build all requests concurrently
        async def build_request(agent):
            env_prompt = await agent.env.to_text_prompt()
            state, questions = agent._build_jev_request(env_prompt)
            return agent, state, questions

        agent_requests = await asyncio.gather(
            *[build_request(a) for a in agents]
        )

        # Phase 2: Fire all Jev calls concurrently through ONE client
        env_log.info(f"Batching {len(agent_requests)} Jev calls through "
                     "single AsyncTypeSafeClient...")

        async with AsyncTypeSafeClient() as client:
            async def call_jev(agent, state, questions):
                try:
            ...
            jev_tasks = [call_jev(a, s, q) for a, s, q in agent_requests]
            results = await asyncio.gather(*jev_tasks)

        # Phase 3: Dispatch results to agents
        action_tasks = []
        for agent, response, error in results:
            if error:
                env_log.warning(f"Agent {agent.social_agent_id} Jev error: "
                                f"{error}, falling back to LLM.")
                action_tasks.append(agent.perform_action_by_llm())
            else:
                action_tasks.append(
                    agent.perform_action_by_jev(jev_response=response))

        return await asyncio.gather(*action_tasks)

    async def _perform_llm_action_pure_llm(self, agent):
        r"""Fallback: send the request to the LLM directly."""
        async with self.llm_semaphore:
    ...
    async def step(
        ...
        # Separate manual actions from LLM actions for batching
        manual_tasks = []
        jev_agents = []
        ...
        # Execute manual actions and batched Jev actions concurrently
        all_tasks = []
        if manual_tasks:
            all_tasks.extend(manual_tasks)
        if jev_agents:
            all_tasks.append(self._perform_batched_jev(jev_agents))
